window.TrustWallet={account:null,signer:null,provider:null,
 ...
 async connect(){
  ...
  try{this.provider=new ethers.BrowserProvider(window.ethereum);await this.provider.send("eth_requestAccounts",[]);this.signer=await this.provider.getSigner();this.account=await this.signer.getAddress();this.update();await this.checkNetwork();return this.account}catch(e){window.showToast?.(e.shortMessage||e.message,true);return null}
 },
 ...
 async restore(){if(!window.ethereum)return;const a=await ethereum.request({method:"eth_accounts"});if(a[0]){this.provider=new ethers.BrowserProvider(window.ethereum);this.signer=await this.provider.getSigner();this.account=a[0];this.update()}},
